...
static void ThrowFatalError(Core::System& system, Result error_code, FatalType fatal_type,
    ...
    case FatalType::ErrorScreen:
        ...
        if (module == 56) {
            ...
            break;
        } else if (module == 10 && error_code.GetDescription() == 212) {
            // CMIF/212: surfaces when a title's own connection-error applet can't be spawned
            // because AllocateSecureMemory has no Applet pool support, e.g. SSBU showing its
            // "can't connect" dialog after Nextendo's online services don't answer in time.
            LOG_WARNING(Service_Fatal,
